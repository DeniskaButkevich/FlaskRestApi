...
from core.routers.endpoints import category_endpoints


def test_create_category(test_client, monkeypatch):
    test_data = {"name": "some category"}

    response = test_client.post("/category/", data=json.dumps(test_data), )
    assert response.status_code == 201
    assert test_data['name'] == response.json()['name']


def test_create_category_invalid_json(test_client):
    response = test_client.post("/category/", data=json.dumps({"title": "something"}))
...
def test_get_category_by_id(test_client, monkeypatch):
    test_data = {"id": 17, "name": "something"}

    def mock_get():
        return test_data

    monkeypatch.setattr(category_endpoints, 'get_category', mock_get())
...
def test_get_category_by_incorrect_id(test_client, monkeypatch):
    ...
    monkeypatch.setattr(category_endpoints, "get", mock_get)
...
def test_get_all_categories(test_client, monkeypatch):
    ...
    monkeypatch.setattr(category_endpoints, "get_all", mock_get_all)
...
def test_update_category(test_client, monkeypatch):
    ...
    monkeypatch.setattr(category_endpoints, "put", mock_patch)
...
def test_remove_note(test_client, monkeypatch):
    ...
    monkeypatch.setattr(category_endpoints, "delete", mock_delete)
